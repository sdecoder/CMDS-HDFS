...
public class DiskChecker {
	 ...
	 * following way: While creating the non-existent parent directories, this
	 * method checks for the existence of those directories if the mkdir fails
	 * at any point (since that directory might have just been created by some
	 * other process). If both mkdir() and the exists() check fails for any
	 * seemingly non-existent directory, then we signal an error; Sun's mkdir
	 * would signal an error (return false) if a directory it is attempting to
	...
	/**
	 * Create the local directory if necessary, check permissions and also
	 * ensure it can be read from and written into.
	 * 
	 * @param localFS
	 *            local filesystem
	 * @param dir
