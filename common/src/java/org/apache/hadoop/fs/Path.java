...
public class Path implements Comparable {
	...
	public Path(URI aUri) {
		...
		try {
			if(aUri.getScheme() == null){
				...
				if (aUri.getScheme().compareTo("hdfs") == 0) {
					uri = new URI(null, null, normalizePath(aUri.getPath()), null, aUri.getFragment()).normalize();
				}else {
					uri = aUri;
				}
			}
	...
	public static final Log LOG = LogFactory.getLog(Path.class);
	private void initialize(String scheme, String authority, String path, String fragment) {
		 
		try {
			if (scheme != null) {
				if (scheme.compareToIgnoreCase("hdfs") == 0) {
					this.uri = new URI(null, null, normalizePath(path), null, fragment).normalize();
					//LOG.info("[inf] Path.initialize:<scheme != null> " + this.uri);
					//LOG.info("[inf] Path.initialize:<scheme != null> " + new URI(scheme, authority, normalizePath(path), null, fragment).normalize());
				} else {
	 ...
	 * named or its implementation is incorrect.
	 */
