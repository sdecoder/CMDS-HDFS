...
public class FsShell extends Configured implements Tool {
   ...
   */
	 ...
	 *                If some IO failed
	 */
	private void copyToLocal(final FileSystem srcFS, final FileStatus srcStatus, final File dst, final boolean copyCrc) throws IOException {
		/*
	...
	private class TextRecordInputStream extends InputStream {
		...
		public TextRecordInputStream(FileStatus f) throws IOException {
			...
			key = ReflectionUtils.newInstance(r.getKeyClass().asSubclass(WritableComparable.class), lconf);
			val = ReflectionUtils.newInstance(r.getValueClass().asSubclass(Writable.class), lconf);
	...
	private int ls(FileStatus src, FileSystem srcFs, boolean recursive, boolean printHeader) throws IOException {
		...
		if (items == null) {
			...
			for (int i = 0; i < items.length; i++) {
				...
				System.out.printf("%" + maxReplication + "s ", (stat.isFile() ? stat.getReplication() : "-"));
				if (maxOwner > 0)
	...
	/**
	 * Show the size of a partition in the filesystem that contains the
	 * specified <i>path</i>.
	 * 
	 * @param path
	 *            a path specifying the source partition. null means /.
	 * @throws IOException
	 */
	...
	private void printUsageSummary(List<UsagePair> usages, boolean humanReadable) {
		...
		for (UsagePair usage : usages) {
			String toPrint = humanReadable ? StringUtils.humanReadableInt(usage.bytes) : String.valueOf(usage.bytes);
	...
	public void fstest() throws IOException {
		System.out.println("[inf] FsShell.fstest: HDFS only");
		 Path hdfspath = new Path("/");
		 FileSystem fileSystem = hdfspath.getFileSystem(this.getConf());	 
		 fileSystem.fstest();
	...
	//add by wangyouwei
	private int dumpNamespace(Configuration conf) throws IOException {
		Path root = new Path("/");
		FileSystem cloverFileSystem = root.getFileSystem(getConf());
		//cloverFileSystem.dumpNamespace();
		return 0;
	}
	 ...
	 * @param skipTrash
	 *            Should we skip the trash, if it's enabled?
	 * @throws IOException
	 * @see org.apache.hadoop.fs.FileSystem#globStatus(Path)
	 */
	...
	int runCmdHandler(CmdHandler handler, String[] args, int startIndex, boolean recursive) throws IOException {
		...
		for (int i = startIndex; i < args.length; i++) {
			...
			// if nothing matches to given glob pattern then increment error
			// count
			if (paths.length == 0) {
	 ...
	 *             {@link org.apache.hadoop.util.StringUtils#byteDesc} instead.
	 */
	@Deprecated
	...
	/**
	 * @deprecated Consider using
	 *             {@link org.apache.hadoop.util.StringUtils#limitDecimalTo2}
	 *             instead.
	 */
	@Deprecated
	...
	public int run(String argv[]) throws Exception {
		...
		try {
			if ("-put".equals(cmd) || "-copyFromLocal".equals(cmd)) {
				...
				// add by wangyouwei
				exitCode = dumpNamespace(getConf());
			} else if ("-dus".equals(cmd)) {
				dus(argv, i);
			} else if (Count.matches(cmd)) {
