 ...
 * to accommodate the file that is being considered for creation. If the space
...
public class LocalDirAllocator {
	...
	/**
	 * Get a path from the local FS. Pass size as SIZE_UNKNOWN if not known
	 * apriori. We round-robin over the set of disks (via the configured dirs)
	 * and return the first complete path which has enough space
	 * 
	 * @param pathStr
	 *            the requested path (this will be created on the first
	 *            available disk)
	...
	private static class AllocatorPerContext {
		...
		public synchronized Path getLocalPathForWrite(String pathStr, long size, Configuration conf) throws IOException {
			...
			// add by wangyouwei
			//LOG.info("[inf] LocalDirAllocator.getLocalPathForWrite: pathStr: " + pathStr);  
			...
			//LOG.info("[inf] returnPath: " + returnPath);
			if (returnPath != null) {
				return returnPath;
			}
		 ...
		 * round-robin over the set of disks (via the configured dirs) and
