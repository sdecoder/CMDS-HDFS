...
class DFSOutputStream extends FSOutputSummer implements Syncable {
  ...
  class DataStreamer extends Daemon {
    ...
    private boolean setupPipelineForAppendOrRecovery() throws IOException {
      // check number of datanodes
    	ClientProtocol namenode = dfsClient.get_namenode_for_parent_path(src);
      ...
      while (!success && !streamerClosed && dfsClient.clientRunning) {
        ...
        LocatedBlock lb = namenode.updateBlockForPipeline(block, dfsClient.clientName);
      ...
      if (success) {
        ...
        namenode.updatePipeline(dfsClient.clientName, block, newBlock, nodes);
    ...
    private DatanodeInfo[] nextBlockOutputStream(String client) throws IOException {
      ...
      do {
        ...
        if (!success) {
          ...
          ClientProtocol namenode = dfsClient.get_namenode_for_parent_path(src);
          namenode.abandonBlock(block, src, dfsClient.clientName);
          block = null;
    ...
    private LocatedBlock locateFollowingBlock(long start,
      ...
      while (true) {
        ...
        while (true) {
          try {
      		ClientProtocol namenode = dfsClient.get_namenode_for_parent_path(src);	  
            return namenode.addBlock(src, dfsClient.clientName, block, excludedNodes);
  ...
  DFSOutputStream(DFSClient dfsClient, String src, FsPermission masked, EnumSet<CreateFlag> flag,
    ...
    try {
    	ClientProtocol namenode = DFSClient.get_namenode_for_parent_path(src);    	
    	namenode.create(
          src, masked, dfsClient.clientName, 
          new EnumSetWritable<CreateFlag>(flag), 
  ...
  public synchronized void hflush() throws IOException {
    ...
    try {
      ...
      if (persistBlocks.getAndSet(false)) {
    	  ClientProtocol namenode = dfsClient.get_namenode_for_parent_path(src);
        namenode.fsync(src, dfsClient.clientName);
      }
    } catch (IOException e) {
  ...
  private void completeFile(Block last) throws IOException {
    ...
    while (!fileComplete) {
    	ClientProtocol namenode = dfsClient.get_namenode_for_parent_path(src);
      fileComplete = namenode.complete(src, dfsClient.clientName, last);
      if (!fileComplete) {
