...
class DataBlockScanner implements Runnable {
  ...
  private void handleScanFailure(Block block) {
    ...
    try {
      DatanodeInfo[] dnArr = { new DatanodeInfo(datanode.dnRegistration.get(datanode.namenode.getNNIndex(block.getBlockId()))) };
      LocatedBlock[] blocks = { new LocatedBlock(block, dnArr) }; 
      datanode.namenode.getNN(block.getBlockId()).reportBadBlocks(blocks);
    } catch (IOException e){
