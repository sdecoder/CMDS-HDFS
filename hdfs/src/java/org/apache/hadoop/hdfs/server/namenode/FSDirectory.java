...
class FSDirectory implements Closeable {
  ...
  void loadFSImage(Collection<URI> dataDirs,
    ...
    if (startOpt == StartupOption.FORMAT) {
      ...
      fsImage.format(fsImage.formatSeed);
  ...
  //=========================================================================
  //add by wangyouwei
  public void unprotected_unlink_inodefile(String src) throws IOException {
		long mtime = FSNamesystem.now();
		src = normalizePath(src);
		NameNode.stateChangeLog.info("unprotectedUnlinkINodeFile: " + src);
		INode[] inodes = rootDir.getExistingPathINodes(src, false);
		INode targetNode = inodes[inodes.length - 1];
		
		if (targetNode == null) { // non-existent src
			NameNode.stateChangeLog.debug("DIR* FSDirectory.unprotectedUnlinkINodeFile: " + "failed to remove " + src + " because it does not exist");
			return;
		}
		
		if (!(targetNode instanceof INodeFile)) {
			throw new IOException("target node to be unlinked is not INodeFile: " + src);
		}
		
		if (inodes.length == 1) { // src is the rofot
			NameNode.stateChangeLog.warn("DIR* FSDirectory.unprotectedUnlinkINodeFile: " + "failed to remove " + src + " because the root is not allowed to be deleted");
			return;
		}
		int pos = inodes.length - 1;
		// Remove the node from the namespace
		...
		for (BlockInfo blk : ((INodeFile)targetNode).getBlocks()) {
			blk.setINode(null);
			getBlockManager().remove_block_without_invalidating(blk);
		}

		// set the parent's modification time
		inodes[pos - 1].setModificationTime(mtime);
		if (NameNode.stateChangeLog.isDebugEnabled()) {
			NameNode.stateChangeLog.debug("DIR* FSDirectory.unprotectedUnlinkINodeFile: " + src + " is removed");
		}
		return;

	}

	public void unlink_inodefile(String src) throws IOException {
		synchronized (rootDir) {
			unprotected_unlink_inodefile(src);
		}
	}
	
	
	public void unprotected_add_inodefile(String src, INodeFile child, long childDiskspace, boolean inheritPermission) throws IOException, QuotaExceededException, UnresolvedLinkException {
		if (child == null) {
			throw new IOException("fsdirectory.unprotectedAddINodeFile: child is null");
		}
		...
		INode[] inodes = new INode[components.length];
		rootDir.getExistingPathINodes(components, inodes, false);
		addChild(inodes, inodes.length - 1, child, childDiskspace, inheritPermission);

		// register its block with inodefile
		BlockInfo[] _biArray_ = child.getBlocks();
		for (BlockInfo _bi : _biArray_) {
			//NameNode.LOG.info("[dbg] blockinfo: " + _bi);
			getBlockManager().addINode(_bi, child);
			int cap = _bi.getCapacity();
			for (int i = 0; i < cap; i++) {
				DatanodeDescriptor d = _bi.getDatanode(i);
				if (d != null) {
					getBlockManager().addBlock(d, _bi, null);
				}
	...
	public void unprotected_add_inodefile_replaying_log(String src, INodeFile child, long childDiskspace, boolean inheritPermission) throws IOException, QuotaExceededException, UnresolvedLinkException {
		if (child == null) {
			throw new IOException("fsdirectory.unprotectedAddINodeFile: child is null");
		}
		//NameNode.LOG.info("[dbg] FSDirectory.unprotected_add_inodefile_replaying_log");
		byte[][] components = INode.getPathComponents(src);
		child.setLocalName(components[components.length - 1]);
		INode[] inodes = new INode[components.length];
		rootDir.getExistingPathINodes(components, inodes, false);
		...
		for (BlockInfo _bi : _biArray_) {
			//NameNode.LOG.info("[dbg] blockinfo: " + _bi);
			for (int i = 0; i < _bi.getCapacity(); i++) {
				_bi.setDatanode(i, null);
				_bi.setPrevious(i, null);
				_bi.setNext(i, null);
			}
	...
	public void add_inodefile(String src, INodeFile child, long childDiskspace, boolean inheritPermission) throws IOException {
		synchronized (rootDir) {
			unprotected_add_inodefile(src, child, childDiskspace, inheritPermission);
	...
	//add by wangyouwei
	boolean rename_without_log(String src, String dst) throws QuotaExceededException, UnresolvedLinkException, FileAlreadyExistsException {
		if (NameNode.stateChangeLog.isDebugEnabled()) {
				NameNode.stateChangeLog.debug("DIR* FSDirectory.renameTo: " + src + " to " + dst);
		}
		waitForReady();
		long now = FSNamesystem.now();
		// comment by wangyouwei:
		// synchronized inside the unprotected method
		if (!unprotectedRenameTo(src, dst, now))
				return false;
	...
	private void unprotected_mkdir_without_log(INode[] inodes, int pos, byte[] name, PermissionStatus permission, boolean inheritPermission, long timestamp) throws QuotaExceededException {
		inodes[pos] = addChild(inodes, pos, new INodeDirectory(name, permission, timestamp), -1, inheritPermission);
	}
	
	//add by wangyouwei
	INode unprotected_mkdir_without_Log(String src, PermissionStatus permissions, long timestamp) throws QuotaExceededException, UnresolvedLinkException {
		// this function only called by
		// org.apache.hadoop.hdfs.server.namenode.FSEditLog.loadEditRecords(int,
		// DataInputStream, boolean)
		// XXX modification needed?
		byte[][] components = INode.getPathComponents(src);
		INode[] inodes = new INode[components.length];
		synchronized (rootDir) {
			rootDir.getExistingPathINodes(components, inodes, false);
			unprotected_mkdir_without_log(inodes, inodes.length - 1, components[inodes.length - 1], permissions, false, timestamp);
			return inodes[inodes.length - 1];
		}
	}

	
	boolean mkdirs_without_log(String src, PermissionStatus permissions,
			boolean inheritPermission, long now)
			throws FileAlreadyExistsException, QuotaExceededException,
			UnresolvedLinkException {
		...
		synchronized (rootDir) {
			...
			for (; i < inodes.length && inodes[i] != null; i++) {
				pathbuilder.append(Path.SEPARATOR + names[i]);
				if (!inodes[i].isDirectory()) {
					throw new FileAlreadyExistsException( "Parent path is not a directory: " + pathbuilder
			...
			for (; i < inodes.length; i++) {
				pathbuilder.append(Path.SEPARATOR + names[i]);
				String cur = pathbuilder.toString();
				unprotected_mkdir_without_log(inodes, i, components[i],
						permissions, inheritPermission
	...
	boolean delete_without_log(String src, List<Block> collectedBlocks) throws UnresolvedLinkException  {
		if (NameNode.stateChangeLog.isDebugEnabled()) {
			NameNode.stateChangeLog.debug("DIR* FSDirectory.delete: " + src);
		}
		waitForReady();
