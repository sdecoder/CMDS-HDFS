...
public class FSNamesystem implements HdfsBlockConst, FSConstants, FSNamesystemMBean, FSClusterStats {
  public static final Log LOG = LogFactory.getLog(FSNamesystem.class);
  protected GDT gdt = new GDT();
  ...
  static Random randBlockId = new Random();
	...
	private long getBlockId() {
		return (FSNamesystem.randBlockId.nextLong() & ~BLOCKID_NODE_MASK) | ((long) NameNode.selfId << BLOCKID_NODE_SHIFT);
	}
  ...
  private Block allocateBlock(String src,
    ...
    Block b = new Block(getBlockId(), 0, 0);
    while(isValidBlock(b)) {
	b.setBlockId(getBlockId());
    }
  ...
  boolean renameTo(String src, String dst) 
	...
	//--------------------------------------------------------------------------------------
	//concurrent control
	//ConcurrentSkipListSet<String> flat_lock_table = new ConcurrentSkipListSet<String>();
	ConcurrentSkipListMap<String, String> flat_lock_table = new ConcurrentSkipListMap<String, String>();
	
	//---------------------------------------------------------------------------------------
	// file system operations
	boolean rename_to_without_log(String src, String dst) throws IOException, UnresolvedLinkException {
		boolean status = rename_internal_without_log(src, dst);
		//getEditLog().logSync();
		if (status && auditLog.isInfoEnabled()) {
			final HdfsFileStatus stat = dir.getFileInfo(dst, false);
  ...
  private synchronized boolean renameToInternal(String src, String dst)
	...
	private synchronized boolean rename_internal_without_log(String src, String dst) throws IOException, UnresolvedLinkException {
		NameNode.stateChangeLog.debug("DIR* NameSystem.renameTo: " + src + " to " + dst);
		if (isInSafeMode())
			throw new SafeModeException("Cannot rename " + src, safeMode);
		if (!DFSUtil.isValidName(dst)) {
		...
		HdfsFileStatus dinfo = dir.getFileInfo(dst, false);
		if (dir.rename_without_log(src, dst)) {
			changeLease(src, dst, dinfo); // update lease with new filename
			return true;
		}
		return false;
	...
	/*public synchronized boolean barrier(long timestamp, long hint) throws IOException {
		return dir.barrier(timestamp, hint);
	}*/
...
public boolean deleteWithoutLog(String src, boolean recursive, List<String> ls) throws IOException, UnresolvedLinkException {
		if ((!recursive) && (!dir.isDirEmpty(src))) {
			throw new IOException(src + " is non empty");
		}
		if (NameNode.stateChangeLog.isDebugEnabled()) {
			NameNode.stateChangeLog.debug("DIR* NameSystem.delete without log: " + src);
		}
		boolean status = delete_internal_without_log(src, true, ls);
		if (status && auditLog.isInfoEnabled()) {
  ...
  private boolean deleteInternal(String src, boolean enforcePermission) 
	...
	//add by wangyouwei
	private boolean delete_internal_without_log(String src, boolean enforcePermission, List<String> ls) throws IOException, UnresolvedLinkException {
		boolean deleteNow = false;
		ArrayList<Block> collectedBlocks = new ArrayList<Block>();
		synchronized (this) {
			if (isInSafeMode()) {
			...
			// Unlink the target directory from directory tree
			if (!dir.delete_without_log(src, collectedBlocks)) {
				return false;
			}
			deleteNow = collectedBlocks.size() <= BLOCK_DELETION_INCREMENT;
			if (deleteNow) { // Perform small deletes right away
...
//add by tacitus
	public boolean mkdirsWithoutLog(String src, PermissionStatus permissions, 
			boolean createParent, long now) throws IOException, UnresolvedLinkException {
		NameNode.stateChangeLog.debug("mkdirs(String " + src + ", PermissionStatus " + permissions + ",  createParent: " + createParent);
		boolean status = mkdirsInternalWithoutLog(src, permissions, createParent, now);
 		if (status && auditLog.isInfoEnabled()) {
			final HdfsFileStatus stat = dir.getFileInfo(src, false);
  ...
  private synchronized boolean mkdirsInternal(String src,
	...
	private synchronized boolean mkdirsInternalWithoutLog(String src, PermissionStatus permissions, 
			boolean createParent, long now) throws IOException, UnresolvedLinkException {
		NameNode.stateChangeLog.debug("DIR* NameSystem.mkdirs: " + src);
		if (isPermissionEnabled) {
			checkTraverse(src);
		...
		if (!dir.mkdirs_without_log(src, permissions, false, now)) {
		//if (!dir.mkdirsWithoutLog(src, permissions, false, now())) {
			throw new IOException("Invalid directory name: " + src);
		}
		return true;
	}
  ...
  DatanodeCommand[] handleHeartbeat(DatanodeRegistration nodeReg,
	...
	cmd = nodeinfo.getValidatedBlocks();
	if (cmd != null) {
		cmds.add(cmd);
	}
  ...
  public synchronized void processReport(DatanodeID nodeID, 
	...
	public synchronized void processReportDiff(DatanodeID nodeID, BlockListAsLongs newReport) throws IOException {
		long startTime = now();
		if (NameNode.stateChangeLog.isDebugEnabled()) {
			NameNode.stateChangeLog.debug("BLOCK* NameSystem.processReportDiff: " + "from " + nodeID.getName() + " " + newReport.getNumberOfBlocks() + " blocks");
		}
		DatanodeDescriptor node = getDatanode(nodeID);
		if (node == null) {
		...
		//LOG.info("[dbg] FSNamesystem.processReportDiff");
		//LOG.info("[dbg] DatanodeID -> " + nodeID);
		//LOG.info("[dbg] BlockListAsLongs -> " + newReport);
		// Check if this datanode should actually be shutdown instead.
		if (shouldNodeShutdown(node)) {
			setDatanodeDead(node);
			throw new DisallowedDatanodeException(node);
		...
		DTCohort.print_current_line();
		blockManager.processReportDiff(node, newReport);
		DTCohort.print_current_line();
		NameNode.getNameNodeMetrics().blockReport.inc((int) (now() - startTime));
	}
  /**
