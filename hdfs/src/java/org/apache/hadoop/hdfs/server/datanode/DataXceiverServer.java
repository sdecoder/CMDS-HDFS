...
class DataXceiverServer implements Runnable, FSConstants {
  ...
  public void run() {
    while (datanode.shouldRun) {
      try {
        ...
        LOG.warn(datanode.dnRegistration.get(0) + ":DataXceiveServer: " 
                                + StringUtils.stringifyException(ie));
      } catch (Throwable te) {
        LOG.error(datanode.dnRegistration.get(0) + ":DataXceiveServer: Exiting due to:" 
  ...
  void kill() {
    ...
    try {
      ...
      LOG.warn(datanode.dnRegistration.get(0) + ":DataXceiveServer.kill(): " 
                              + StringUtils.stringifyException(ie));
    }
