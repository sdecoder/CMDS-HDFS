...
public class INodeFile extends INode implements Writable {
  ...
  public void write(DataOutput out) throws IOException{
	  	super.write(out);
		this.getPermissionStatus().write(out);
		out.writeShort(this.blockReplication);
		out.writeLong(modificationTime);
		out.writeLong(accessTime);
		out.writeLong(preferredBlockSize);
		BlockInfo[] blockinfos = this.getBlocks();
		out.writeInt(blockinfos.length);
		for (int i = 0; i < blockinfos.length; i++) {
			blockinfos[i].write(out);
			out.writeInt(blockinfos[i].getCapacity());
			for (int j = 0; j < blockinfos[i].getCapacity(); j++) {
				DatanodeDescriptor dnd = blockinfos[i].getDatanode(j);
				if (dnd == null) {
 					dnd = new DatanodeDescriptor(new DatanodeID("-1"));
				}
				dnd.write(out);
			}
  ...
  public void readFields(DataInput in) throws IOException{
	  super.readFields(in);
	  PermissionStatus permissionStatus = PermissionStatus.read(in);
	  short replication = in.readShort();
	  long modificationTime = in.readLong();
	  long accessTime = in.readLong();
	  long preferredBlockSize = in.readLong();
	  int nrblocks = in.readInt();
	  BlockInfo[] blocks = new BlockInfo[nrblocks];
	  for (int i = 0; i < blocks.length; i++) {
		Block reg_block = new Block();
		reg_block.readFields(in);
		int capacity = in.readInt();
 		blocks[i] = new BlockInfo(reg_block, capacity);
 		for (int j = 0; j < capacity; j++) {
			//DatanodeInfo dni = DatanodeDescriptor.read(dib);
			DatanodeInfo dni = DatanodeDescriptor.read(in);
			if (dni.getName().equals("-1")) {
				blocks[i].setDatanode(j, null);
			}else {
				DatanodeDescriptor dnd = new DatanodeDescriptor(dni);
				blocks[i].setDatanode(j, dnd);
				dnd.addBlock(blocks[i]);
			}
				//}
		}
	  ...
	  super.setPermissionStatus(permissionStatus);
