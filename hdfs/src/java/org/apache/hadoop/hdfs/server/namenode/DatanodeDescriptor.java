...
public class DatanodeDescriptor extends DatanodeInfo implements HdfsBlockConst {
  ...
  private Set<Block> invalidateBlocks = new TreeSet<Block>();
	/** A set of blocks to be validated by this datanode */
	private Set<Block> validatedBlocks = new TreeSet<Block>();
  ...
  void addBlocksToBeInvalidated(List<Block> blocklist) {
	...
	void addBlockToBeValidated(Block b) {
		synchronized (validatedBlocks) {
			validatedBlocks.add(b);
		}
	}
  ...
  int getNumberOfBlocksToBeInvalidated() {
	...
	int getNumberofBlocksToBeValidated() {
		synchronized (validatedBlocks) {
  ...
  BlockCommand getInvalidateBlocks(int maxblocks) {
	...
	/**
	 * Remove all the blocks to be validated
	 */
	BlockCommand getValidatedBlocks() {
		Block[] validatedList = getBlockArray(validatedBlocks, getNumberofBlocksToBeValidated());
		return validatedList == null ? null : new BlockCommand(DatanodeProtocol.DNA_VALIDATE, validatedList);
	}
  ...
  void reportDiff(BlockManager blockManager,
	...
	void reportDiffNoRemove(BlockManager blockManager, BlockListAsLongs newReport, Collection<Block> toAdd, 
		...
		//FSNamesystem.LOG.info("[inf] FSNamesystem.reportDiff: ");
		BlockInfo delimiter = new BlockInfo(new Block(), 1);
		boolean added = this.addBlock(delimiter);
		assert added : "Delimiting block cannot be present in the node";
		if (newReport == null)
		...
		while (itBR.hasNext()) {
			Block iblk = itBR.next();
			ReplicaState iState = itBR.getCurrentReplicaState();
			BlockInfo storedBlock = processReportedBlock(blockManager, iblk, iState, toAdd, toInvalidate, toCorrupt);			
			// move block to the head of the lists			
			if (storedBlock != null && storedBlock.findDatanode(this) >= 0) {				
				this.moveBlockToHead(storedBlock);
			}
		}
 ...
 /**
  ...
  BlockInfo processReportedBlock(
    ...
    if(storedBlock == null) {
      ...
      // FIXME: for clover, an invalidated block may means that this block
      // has been renamed to other NN.

      toInvalidate.add(new Block(block));
      return null;
    }

		// FIXME: block processing for clover
		int node = (int) ((block.getBlockId() & BLOCKID_NODE_MASK) >> BLOCKID_NODE_SHIFT);

		if (node != NameNode.selfId) {
			// add this block to validated list
			addBlockToBeValidated(block);
		}
