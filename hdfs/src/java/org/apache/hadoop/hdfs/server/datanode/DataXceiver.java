...
class DataXceiver extends DataTransferProtocol.Receiver
  ...
  public void run() {
    ...
    try {
      ...
      LOG.error(datanode.dnRegistration.get(0) + ":DataXceiver",t);
    } finally {
      if (LOG.isDebugEnabled()) {
        LOG.debug(datanode.dnRegistration.get(0) + ":Number of active connections is: "
            + datanode.getXceiverCount());
      }
  ...
  protected void opReadBlock(DataInputStream in,
    ...
    int idx = datanode.namenode.getNNIndex(blockId);
    
    if (datanode.isAccessTokenEnabled[idx]
        && !datanode.accessTokenHandler.get(idx).checkAccess(accessToken, null, blockId,
    ...
    final String clientTraceFmt =
      clientName.length() > 0 && ClientTraceLog.isInfoEnabled()
        ? String.format(DN_CLIENTTRACE_FORMAT, localAddress, remoteAddress,
            ...
            datanode.dnRegistration.get(idx).getStorageID(), block, "%d")
        : datanode.dnRegistration.get(idx) + " Served block " + block + " to " +
            s.getInetAddress();
    try {
      try {
       ...
       */
      LOG.warn(datanode.dnRegistration.get(idx) +  ":Got exception while serving " + 
          block + " to " +
                s.getInetAddress() + ":\n" + 
  ...
  protected void opWriteBlock(DataInputStream in, long blockId, long blockGs,
    ...
    if (datanode.isAccessTokenEnabled[idx]
        && !datanode.accessTokenHandler.get(idx).checkAccess(accessToken, null, block
            .getBlockId(), AccessTokenHandler.AccessMode.WRITE)) {
      try {
        if (client.length() != 0) {
          ERROR_ACCESS_TOKEN.write(replyOut);
          Text.writeString(replyOut, datanode.dnRegistration.get(idx).getName());
          replyOut.flush();
        }
        throw new IOException("Access token verification failed, for client "
      ...
      if (targets.length > 0) {
        ...
        try {
          ...
          if (client.length() > 0) {
            ...
            LOG.info(datanode.dnRegistration.get(idx) + ":Exception transfering block " +
