...
public class SecondaryNameNode implements Runnable {
  ...
  public enum CloverRoutingMode {BCAST, HASH};
  
  public class CloverRouting {
	  private ArrayList<NamenodeProtocol> availableNNs;
	  private CloverRoutingMode mode;
	  private long seed;
	  
	  public CloverRouting(ArrayList<NamenodeProtocol> dnnList) {
		  this(dnnList, CloverRoutingMode.HASH);
	  }
	  
	  public CloverRouting(ArrayList<NamenodeProtocol> dnnList, CloverRoutingMode mode) {
	  ...
	  public void setMode(CloverRoutingMode mode) {
		  ...
		  switch (mode) {
		  case BCAST: modeStr += "BCAST";
		  break;
		  case HASH: modeStr += "HASH";
		  break;
		  default: modeStr += "HASH"; mode = CloverRoutingMode.HASH;
		  break;
		  }
		  LOG.info(modeStr);
		  this.mode = mode;
	  ...
	  public String getCurrentAddr(int idx) {
		  ArrayList<URI> al = (ArrayList<URI>)dnnNames;
	  ...
	  public int getIndex(NamenodeProtocol np) {
		  for (int i = 0; i < availableNNs.size(); i++) {
			  if (np == availableNNs.get(i)) {
				  return i;
			  }
		  }
		  return -1;
	  ...
	  public NamenodeProtocol getNNPhy(int idx) {
		  if (idx < 0 || idx >= availableNNs.size()) {
			  return null;
		  } else 
	  ...
	  public int hash(long id) {
		  return (int)(id % this.seed);
	  }
	  
	  public NamenodeProtocol getNN(String src) throws IOException {
		  switch (mode) {
		  case BCAST:
		  ...
		  default:
			  String dirname;
		  	  if (src == null || src.equals("") || !src.contains("/"))
		  		  dirname = "/";
		  	  else {
		  		  dirname = src.substring(0, src.lastIndexOf('/'));
		  		  if (dirname == null || dirname.equals("")) {
		  			  dirname = "/";
		  		  }
		  	  }
		  	  LOG.info("Got dirname of '" + src + "' => '" + dirname);
		  	  // find the UID
		  	  DFSClient client = new DFSClient(conf);
		  	  long uid = client.get_uid(src);
		  	  //long uid = getNN3(dirname).getUID(dirname);
		  	  if (uid == -1L)
		  		  throw new FileNotFoundException("getUID(" + dirname + "): file doesn't exist.");
		  	  return availableNNs.get(hash(uid) % availableNNs.size());
		  }
	  }
	  
	  public NamenodeProtocol getNN2(String src) throws IOException {
		  switch (mode) {
		  case BCAST:
			  // must fail to notify our users
			  return null;
		  ...
		  default:
			  if (src == null || src.equals("") || !src.contains("/"))
				  src = "/";
		  	  LOG.info("Got dirname of '" + src + "'");
		  	  // find the UID
		  	  DFSClient client = new DFSClient(conf);
		  	  long uid = client.get_uid(src);
		  	  //long uid = getNN3(src).getUID(src);
		  	  if (uid == -1L)
		  		  throw new FileNotFoundException("getUID(" + src + "): file doesn't exist.");
		  	  return availableNNs.get(hash(uid) % availableNNs.size());
		  }
	  }
	  
	  public NamenodeProtocol getNN3(String src) {
		  return availableNNs.get((src.hashCode() & 0xfffffff) % availableNNs.size());
	  }
  }
  
  // fields of DNN
  public CloverRouting dnn;
  public Collection<URI> dnnNames;
  ...
  public Collection<NamenodeProtocol> getDnn(Collection<URI> uri, Configuration conf) throws IOException {
	  Collection<NamenodeProtocol> dnns = new ArrayList<NamenodeProtocol>();
	  for (URI nn : uri) {
		  if (nn.getAuthority().equalsIgnoreCase(FileSystem.getDefaultUri(conf).getAuthority())) {
			  LOG.info("Try to connect to NN: " + nn + " via LOCAL.");
			  dnns.add(namenode);
		  } else {
			  LOG.info("Try to connect to NN: " + nn + " via REMOTE.");
			  dnns.add((NamenodeProtocol)RPC.waitForProxy(
					  NamenodeProtocol.class,
					  NamenodeProtocol.versionID,
					  NameNode.getAddress(nn.getAuthority()),
					  conf));
		  }
	  }
	  return dnns;
  ...
  public void putDnn() {
	  for (int i = 0; i < dnnNames.size(); i++) {
		  RPC.stopProxy(dnn.getNNPhy(i));
	  }
  }
  ...
  private void initialize(Configuration conf) throws IOException {
    ...
    this.dnnNames = getDnnURI(conf);
    LOG.info("Get DNN List: " + dnnNames);
    dnn = new CloverRouting((ArrayList<NamenodeProtocol>)getDnn(this.dnnNames, conf));
