...
public class TestReplication extends TestCase {
  ...
  public void testBadBlockReportOnTransfer() throws Exception {
    ...
    blocks = dfsClient.getNamenode().getNN(file1.toString()).
  ...
  public void testPendingReplicationRetry() throws IOException {
    ...
    try {
      ...
      waitForBlockReplication(testFile, dfsClient.getNamenode().getNN(testFile.toString()), numDataNodes, -1);

      // get first block of the file.
      String block = dfsClient.getNamenode().getNN(testFile.toString()).
  ...
  private void changeBlockLen(MiniDFSCluster cluster, 
    ...
    LocatedBlocks blocks = dfsClient.getNamenode().getNN(fileName.toString()).getBlockLocations(
    ...
    if (lenDelta < 0) { // replica truncated
    	while (!blocks.get(0).isCorrupt() || 
    		...
    		Thread.sleep(100);
    		blocks = dfsClient.getNamenode().getNN(fileName.toString()).getBlockLocations(
    				fileName.toString(), 0, fileLen);
    	}
